...
export async function searchLeetCodeProblemsImpl(queryInput: string) {
	const fromUrlSlug = tryExtractLeetCodeSlug(queryInput);
		...
		if (cachedResults) {
			console.info("[leetcode-search] cache hit", searchCacheKey);
		...
		console.info(
			"[leetcode-search] status",
			response.status,
			response.statusText,
		);
		try {
			const parsedForLog = JSON.parse(rawText);
			console.info(
				"[leetcode-search] response",
				JSON.stringify(parsedForLog, null, 2),
			);
		} catch {
			console.info("[leetcode-search] raw response", rawText);
		}
		...
		const ranked = payload
			...
			.filter((question) =>
				Boolean(question.title && question.slug && question.url),
			)
			.slice(0, 10)
			.sort((a, b) => {
				...
				const aPrefix =
					a.slug.startsWith(slugQuery) || aTitle.startsWith(query);
				const bPrefix =
					b.slug.startsWith(slugQuery) || bTitle.startsWith(query);
